...
def GetCONFIG(CONFIGURATION):
    ...
    PATH = os.path.join(os.path.expanduser('~'), 'dev_ws', 'src', 'ros2_SimRealRobotControl', 'packages', 'irb120')
...
def generate_launch_description():
    ...
    # ros2_control:
    ros2_controllers_path = os.path.join(get_package_share_directory("ros2srrc_robots"), "irb120", "config", "controller.yaml")
    ...
    if EE == "false":
        robot_description_semantic_config = load_file("ros2srrc_irb120_moveit2", "config/irb120.srdf")
    else:
        robot_description_semantic_config = load_file("ros2srrc_irb120_moveit2", "config/irb120" + CONFIGURATION["ee"] + ".srdf")
    ...
    if EE == "false":
        ...
        MoveInterface = Node(
            ...
            executable="move",
            output="screen",
            parameters=[robot_description, robot_description_semantic, kinematics_yaml, {"ROB_PARAM": "irb120"}, {"EE_PARAM": CONFIGURATION["ee"]}, {"ENV_PARAM": "bringup"}],
        )
        SequenceInterface = Node(
            name="sequence",
            package="ros2srrc_execution",

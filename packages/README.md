...
The /packages folder contains standard ROS 2 packages for each robot arm included in ros2_SimRealRobotControl. These packages provide simple simulation environments where a robot (and end-effector, if applicable) is placed on top of a basic robot stand.
...
In addition, the MoveIt!2 configuration files are centralized inside the ros2srrc_moveit package. A single shared _.rviz_ file is used to visualize and operate all single-arm robot configurations in RViz, while the _.srdf_ files remain standardized for each robot or robot+end-effector combination. Some URDF variants that share the same robot and end-effector can reuse the same SRDF configuration files.

Features of this modular setup:

- No redefinition is needed: These parameters do not need to be redefined or replicated in the ROS 2 GZ Sim or MoveIt!2 packages. The packages simply reference the existing parameters.
...
- irb6640_2: ABB IRB-6640 + Zimmer GPP5010NC-00-A Parallel Gripper on top of Robot Stand.

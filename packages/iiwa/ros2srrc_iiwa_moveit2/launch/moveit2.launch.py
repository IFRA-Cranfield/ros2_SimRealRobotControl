...
# moveit2.launch.py:
# Launch file for the KUKA LBR-IIWA Robot GAZEBO SIMULATION + MoveIt!2 Framework in ROS2 Humble:
...
def generate_launch_description():
    ...
    # ========== CELL INFORMATION ========== #
    print("")
    print("===== KUKA LBR-IIWA: Robot Simulation + MoveIt!2 Framework (ros2srrc_iiwa_moveit2) =====")
    ...
    if EE == "false":
        robot_description_semantic_config = load_file("ros2srrc_iiwa_moveit2", "config/iiwa.srdf")
    else:
        robot_description_semantic_config = load_file("ros2srrc_iiwa_moveit2", "config/iiwa" + CONFIGURATION["ee"] + ".srdf")
    
    robot_description_semantic = {"robot_description_semantic": robot_description_semantic_config}

    # Kinematics.yaml file:
    kinematics_yaml = load_yaml("ros2srrc_robots", "iiwa/config/kinematics.yaml")
    robot_description_kinematics = {"robot_description_kinematics": kinematics_yaml}

    # joint_limits.yaml file:
    if EE == "false":
        joint_limits_yaml = load_yaml("ros2srrc_robots", "iiwa/config/joint_limits.yaml")
    else:
        YAML_ROB = load_yaml("ros2srrc_robots", "iiwa/config/joint_limits.yaml")["joint_limits"]
    ...
    pilz_cartesian_limits_yaml = load_yaml("ros2srrc_robots", "iiwa/config/pilz_cartesian_limits.yaml")
    pilz_cartesian_limits = {'robot_description_planning': pilz_cartesian_limits_yaml}

    # MoveIt!2 Controllers:
    if EE == "false":
        moveit_simple_controllers_yaml = load_yaml("ros2srrc_robots", "iiwa/config/controller_moveit2.yaml")
    else:
        YAML_ROB = load_yaml("ros2srrc_robots", "iiwa/config/controller_moveit2.yaml")
    ...
    # RVIZ:
    rviz_base = os.path.join(get_package_share_directory("ros2srrc_iiwa_moveit2"), "config")
    if EE == "false":
        rviz_full_config = os.path.join(rviz_base, "iiwa_moveit2.rviz")
    else:
        rviz_full_config = os.path.join(rviz_base, "iiwa" + CONFIGURATION["ee"] + "_moveit2.rviz")
    ...
    if EE == "false":
        ...
        MoveInterface = Node(
            ...
            executable="move",
            output="screen",
            parameters=[robot_description, robot_description_semantic, kinematics_yaml, {"use_sim_time": True}, {"ROB_PARAM": "iiwa"}, {"EE_PARAM": "none"}, {"ENV_PARAM": "gazebo"}],
        )
        ...
        MoveInterface = Node(
            ...
            executable="move",
            output="screen",
            parameters=[robot_description, robot_description_semantic, kinematics_yaml, {"use_sim_time": True}, {"ROB_PARAM": "iiwa"}, {"EE_PARAM": CONFIGURATION["ee"]}, {"ENV_PARAM": "gazebo"}],
        )
        SequenceInterface = Node(
            name="sequence",
            package="ros2srrc_execution",
    ...
    RobMoveInterface = Node(
        ...
        executable="robmove",
        output="screen",
        parameters=[robot_description, robot_description_semantic, kinematics_yaml, {"use_sim_time": True}, {"ROB_PARAM": "iiwa"}],
    )

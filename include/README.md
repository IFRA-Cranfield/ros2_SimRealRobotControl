...
The improvement of the move_group_interface consists of a small change in the header file that declares the Move Group Interface and its variables. In this repository, Robot/Gripper triggers are executed by making service/action calls that execute different MoveIt!2 commands using the Move Group C++ Interface.
...
The figure below illustrates the process of calling the "Move Group Interface - Plan + Move" method in order to execute a Robot/Gripper trigger from a C++ file containing a ROS 2 Service or Action Server, before and after making the improvement.
...
The small improvement of the move_group_interface.h file consists of adding an empty constructor to the MoveGroupInterface class (line 149 of the move_group_interface_improved.h file). Before adding this element, the MoveGroupInterface class (which defines the whole MoveIt!2 framework of the Robot/Gripper being controlled) could not be declared as an empty global variable, and this is absolutely necessary because MoveGroupInterface must be accessed twice in two different functions throughout the execution of the program:
...
- In the callback function of the ROS 2 service/action. The MoveGroupInterface element is used to execute the trigger (to send commands to MoveIt!2).

Thus, the fact that MoveGroupInterface is declared as a global variable avoids having to define and assign all the attributes (which take some time for MoveIt!2 to process) every single time the ROS 2 service/action is called and the callback function is executed. It has been estimated that about 1-1.5 seconds are saved every single time a trigger is executed thanks to this contribution.
